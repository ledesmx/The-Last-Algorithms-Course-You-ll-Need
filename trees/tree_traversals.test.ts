...
import { pre_order_search, in_order_search, post_order_search } from "./tree_traversals";
...
describe("#in_order_search", () => {
  ...
  test("Unbalanced tree", () => {
    ...
    expect(in_order_search(tree)).toEqual([9, 7, 8, 1, 3, 8, 6, 0, 7, 5, 8, 3, 2]);
  });
});
...
describe("#post_order_search", () => {
  ...
  test("Unbalanced tree", () => {
    ...
    expect(post_order_search(tree)).toEqual([9, 8, 7, 3, 1, 6, 7, 8, 5, 0, 2, 3, 8]);
  });
});
